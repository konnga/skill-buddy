...
function registerIpc(): void {
  ...
  ipcMain.handle(
    ...
    async (_event, name: string, targets: InstallTarget[], enabled: boolean) =>
      runTargets(targets, async (t) => {
        const roots = t.scope === 'project' && t.projectRoot ? [t.projectRoot] : []
        const installation = (await scanInstalledSkills(roots)).find(
          (item) =>
            item.skill.name === name &&
            item.agent === t.agent &&
            item.scope === t.scope &&
            (item.projectRoot ?? '') === (t.projectRoot ?? ''),
        )
        ...
        return setAdapterEnabled(name, enabled, t)
      }),
  )
