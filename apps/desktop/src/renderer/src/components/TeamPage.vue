...
import { Boxes, GitBranch, PackageCheck, RefreshCw, ServerCog, Settings2, Sparkles, Users } from '@lucide/vue'
...
const activeTab = shallowRef<'bundles' | 'skills' | 'mcp' | 'manage'>('bundles')
...
const refreshing = computed(() => loading.value || projectsLoading.value)

async function refreshTeam(): Promise<void> {
  await Promise.all([syncAll(), refreshProjects()])
}
  ...
  <div class="mx-auto flex max-w-3xl flex-col gap-4 px-6 py-6">
    ...
    <template v-else>
      ...
      <div v-if="catalogs.length === 0" class="flex items-center gap-3 rounded-md border border-dashed px-4 py-4 text-sm text-muted-foreground">
        ...
        <span class="flex-1">团队库已配置但还没有可用资源。请进入“管理”创建第一个 Skill、MCP 或岗位包。</span>
      ...
      <TeamLibraryManagementPanel v-else />
    </template>
  </div>
