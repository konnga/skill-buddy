    ...
    <DialogPortal>
      ...
      <DialogContent class="fixed left-1/2 top-1/2 z-50 flex max-h-[90vh] w-[min(820px,calc(100vw-32px))] -translate-x-1/2 -translate-y-1/2 flex-col rounded-lg border bg-background shadow-xl outline-none">
        <div class="border-b px-5 py-4">
          <DialogTitle class="text-base font-semibold">{{ initial ? t('team.bundleEditorEditTitle') : t('team.bundleEditorCreateTitle') }}</DialogTitle>
          <DialogDescription class="mt-1 text-sm text-muted-foreground">{{ t('team.bundleEditorHint') }}</DialogDescription>
        </div>
          ...
          <div class="min-h-0 flex-1 space-y-4 overflow-y-auto px-5 py-4">
            ...
            <div class="grid gap-4 lg:grid-cols-2">
              <section class="overflow-hidden rounded-md border">
                ...
                <p v-if="!skills.length" class="px-3 py-6 text-center text-sm text-muted-foreground">{{ t('team.projectNoSkills') }}</p>
              </section>
              <section class="overflow-hidden rounded-md border">
                ...
                <p v-if="!mcpServers.length" class="px-3 py-6 text-center text-sm text-muted-foreground">{{ t('team.projectNoMcp') }}</p>
              </section>
            </div>
          </div>
